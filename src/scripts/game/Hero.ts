...
export type animType = 'stand' | 'goLeft' | 'goRight' | 'sliceUp' | 'sliceLeft' | 'sliceRight';
export type animData = {key: animType, prior: number, sound?: string};

const alignArcade = (screen: number, fract: number) => {return Math.floor(screen*fract)};
...
export default class Hero {
  ...
  constructor(game: Phaser.Scene) {
  	this._game = game;
    this.sprite = this._game.physics.add.sprite(0,0, "GameAtlas", "knight/knight iso char_idle_0.png").setScale(3.0);
  ...
  private initKnightAnimations(): void{
      for (const anim of animList){
          this._game.anims.create({key: anim.name, frameRate: anim.frameRate, repeat: anim.repeat, frames: this._game.anims.generateFrameNames('GameAtlas', {
            prefix: anim.prefix,
            suffix: ".png",
            start: anim.start,
            end: anim.end
  ...
  public sliceType(object: Phaser.Physics.Arcade.Sprite, hero:Phaser.Physics.Arcade.Sprite){
  	var key: animType;
    if(object.y>hero.y){
  ...
  private animate(anim: animData): void{
  	...
  	if(anim.sound){
  		this._game._sounds.play(anim.sound);
  	}
