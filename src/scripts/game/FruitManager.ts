...
const preSufString = (prefix: string, suffix: string, value: string) => {return prefix+value+suffix};
const randint = (min: number, max:number) => {return Math.floor(Math.random()*(max-min)) + min};

export type atlasConfigData = {key: string, prefix: string, suffix: string};
export type levelConfig = {minGap: number, levelTextures: string[], fruitSpeed: number, rotateSpeed: number};
...
export default class FruitManager{
  ...
  constructor(game: Phaser.Scene, textures: string[], config: atlasConfigData){
  	this._game = game;
  	...
  	this._currentLevel = this._game._level.level;
  ...
  private createFruits(): void{
  	var gap:number = -100;
  	for(let i=0; i<100; i++){
  		gap -= randint(this._levelConfig.minGap, this._levelConfig.minGap + randint(0,50));
  		this.addFruit(randint(this._offSet, this._game.scale.width - this._offSet), gap);  
  	} 

  }

  private addFruit(x: number, y:number, name: string = this.randomTexture(this._levelConfig.levelTextures)): void{
  	var fruit = this._game.physics.add.sprite(
  		x,
  		y,
  		this._atlasConfig.key, 
  		preSufString(
  			this._atlasConfig.prefix,
  			this._atlasConfig.suffix,
  			name)
  		).setScale(3.0);
  ...
  public updateFruits(): void{
  	this._group.getChildren().forEach(child => {
  		child.y += this._levelConfig.fruitSpeed;
  	});
