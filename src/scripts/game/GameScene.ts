...
export default class GameScene extends Phaser.Scene {
  ...
  static readonly fruitTextures: string[] = [
    "Apple",
    "Avocado",
    "Bread",
    "Brownie",
  ...
  static readonly startingLevel: number = 1;
  ...
  preload() {
    ...
    this.load.bitmapFont(
      "arial-stroke",
      "fonts/arial-stroke.png",
      "fonts/arial-stroke.fnt"
    );
  ...
  create(data: { dispatch: GameDispatch; initialState: GameState }) {
    ...
    this.hero.alignHero(
      GameScene.heroStartPos.x,
      GameScene.heroStartPos.y,
      false
    );

    // CREATE FRUITS GROUP
    this.fruits = new FruitManager(
      this,
      GameScene.startingLevel,
      GameScene.fruitTextures,
      { key: "GameAtlas", prefix: "fruits/", suffix: ".png" }
    );

    // LEVEL MANAGAER
    this._level = new LevelManger(
      this,
      GameScene.startingLevel,
      this.newLevelCallback
    );
  ...
  resize() {
    // fixing hero posY and background on resize
    Align.fillScreen(this.bg);
    this.hero.alignHero(
      GameScene.heroStartPos.x,
      GameScene.heroStartPos.y,
  ...
  update() {
    ...
    // check last children height
    if (this.fruits.verify(this.scale.height)) {
      this._sounds.play("fail", this._soundConfig);
      let child = this.fruits.group.getChildren()[0];
      child.destroy();
